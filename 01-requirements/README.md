...
| BR8 | A completed invoice should contain at least one invoice detail. This rule is ensured by the sales workflow rather than a table constraint. |
...
| BR14 | `INVOICE_DETAIL.unit_price` stores the selling price at the time of sale. A new detail copies the current product price; increasing an existing detail retains its original selling price. |
...
| BR18 | The demonstrated system mainly focuses on completed sales invoices. |
| BR19 | `payment_method` must use fixed numeric codes. |
| BR20 | Product stock quantity must not be negative. |
| BR21 | When an invoice detail is inserted or its quantity is increased, a database trigger automatically adjusts the related product stock quantity. |
...
| Giảm tồn kho sau khi bán | Stored procedure kiểm tra tồn kho; trigger trên `INVOICE_DETAIL` thực hiện cập nhật tồn kho sau `INSERT` hoặc `UPDATE quantity` |
| Tạo invoice kèm detail | Workflow tạo hóa đơn cần thêm ít nhất một detail trước khi xem hóa đơn là hoàn tất |

## 9. Assignment Scope and Assumptions

This is an introductory database assignment. Advanced concurrent transaction handling and multi-user conflict resolution are outside the project scope.

Sales invoice details are expected to be added through the provided stored procedure. The procedure validates the input and available stock, while the inventory trigger performs the sale-related adjustment to `PRODUCT.stock_quantity`.
